...
Each article is a folder under `src/content/articles/`, named with a short slug — that
slug becomes the article's URL. A folder needs an `index.md` with frontmatter, and
optionally an image sitting right next to it.

```
src/content/articles/moose-toll-booth/
├── index.md
...
Look at the existing folders in `src/content/articles/` for more working examples.

The home page lists every non-draft article (newest first) with its headline, image,
and excerpt. Each links to a full article page at `/articles/<slug>/`. Articles are also
browsable by year and month via the top nav and at `/archive/<year>/` and
`/archive/<year>/<month>/`.
